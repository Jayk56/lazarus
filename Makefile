...
.PHONY: all build test test-race vet fmt-check check validate-source container helm-lint helm-template helm-digest-negative yaml-check openapi-check ansible-check aap-syntax-check shell-check workflow-check clean
...
validate-source: check test-race helm-lint helm-template helm-digest-negative yaml-check openapi-check ansible-check shell-check workflow-check
...
ansible-check:
	@for playbook in examples/ansible/*.yml; do \
